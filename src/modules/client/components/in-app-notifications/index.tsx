...
export default function ClientNotifications() {
  ...
  const { mutate: deleteNotifications, isPending: isDeleting } = useDeleteNotifications("client")
  const { mutate: deleteAllNotifications, isPending: isDeletingAll } = useDeleteAllNotifications("client")
  ...
  const allSelected = selectAll || selected.size === totalCount
  ...
  const notificationLink = (n: any) => {
    if (n.resource_type === "bug") return `/client/account/my-bugs?bugId=${n.resource_id}`
    if (n.resource_type === "submission") return `/client/account/developer-submissions?submissionId=${n.resource_id}`
          ...
          {notifications.map((n) => (
            ...
            >
              <div className="flex items-start gap-x-3">
                <input
                  type="checkbox"
                  checked={selectAll || selected.has(n.id)}
